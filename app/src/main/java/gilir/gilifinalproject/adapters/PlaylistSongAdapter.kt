...
import gilir.gilifinalproject.databinding.PlaylistSongItemBinding
import gilir.gilifinalproject.models.playlistsong.PlaylistSong
import gilir.gilifinalproject.models.songsapi.Song
import java.util.concurrent.TimeUnit

class PlaylistSongAdapter(private val songs: List<PlaylistSong>, val onFavoriteClicked: (PlaylistSong) -> Unit) : Adapter<PlaylistSongAdapter.SongViewHolder>() {

    class SongViewHolder(val binding: PlaylistSongItemBinding) : ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SongViewHolder {
        val context = LayoutInflater.from(parent.context)
        val binding = PlaylistSongItemBinding.inflate(context, parent, false)
